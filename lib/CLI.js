...
CLI._deepRestart = function(cmd, opts, cb) {
  if (Utility.isConfigFile(cmd)) {
    CLI.actionFromJson('deleteProcessId', cmd, commander, 'file', function() {
      CLI._startJson(cmd, opts, 'restartProcessId', cb);
    });
  }
  else {
    CLI._operate('deleteProcessId', cmd, function() {
      CLI._startScript(cmd, opts, cb);
    });
  }
};
